...
namespace CompanyCode.ApiTemplate.Api.ExtensionMethods
{
    public static class AddSwaggerGenConfigurationExtension
    {
        private class PascalCasePropertyFilter : ISchemaFilter {
            ...
            {
                model.Properties = model.Properties.ToDictionary(d => d.Key.Substring(0, 1).ToUpper() + d.Key.Substring(1), d => d.Value);
            }
        ...
        public static void AddSwaggerGenConfiguration(this IServiceCollection services, CommonAppSettings commonAppSettings)
        {
            services.AddSwaggerGen(c =>
            {
                ...
                c.SwaggerDoc("v1", new OpenApiInfo { Title = commonAppSettings.ApplicationName, Version =  $"v1-{commonAppSettings.ApplicationVersion}" });
                string xmlFile = $"{Assembly.GetExecutingAssembly().GetName().Name}.xml";
                string xmlPath = Path.Combine(AppContext.BaseDirectory, xmlFile);
                c.IncludeXmlComments(xmlPath);
            });
